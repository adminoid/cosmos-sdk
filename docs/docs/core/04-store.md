...
```go reference
https://github.com/adminoid/cosmos-sdk/blob/v0.46.0/store/types/store.go#L16-L19
```
...
```go reference
https://github.com/adminoid/cosmos-sdk/blob/v0.46.0/store/types/store.go#L247-L277
```
...
```go reference
https://github.com/adminoid/cosmos-sdk/blob/v0.46.0/store/types/store.go#L30-L34
```

The `Committer` is an interface that defines methods to persist changes to disk:

```go reference
https://github.com/adminoid/cosmos-sdk/blob/v0.46.0/store/types/store.go#L21-L28
```
...
```go reference
https://github.com/adminoid/cosmos-sdk/blob/v0.46.0/store/types/store.go#L97-L133
```
...
```go reference
https://github.com/adminoid/cosmos-sdk/blob/v0.46.0/store/types/store.go#L141-L187
```

As for concrete implementation, the [`rootMulti.Store`] is the go-to implementation of the `CommitMultiStore` interface.

```go reference
https://github.com/adminoid/cosmos-sdk/blob/v0.46.0/store/rootmulti/store.go#L38-L61
```
...
Whenever the `rootMulti.Store` needs to be branched, a [`cachemulti.Store`](https://github.com/adminoid/cosmos-sdk/blob/v0.46.0/store/cachemulti/store.go) is used.

```go reference
https://github.com/adminoid/cosmos-sdk/blob/v0.46.0/store/cachemulti/store.go#L20-L36
```
...
```go reference
https://github.com/adminoid/cosmos-sdk/blob/v0.46.0/store/types/store.go#L192-L226
```
...
```go reference
https://github.com/adminoid/cosmos-sdk/blob/v0.46.0/x/bank/keeper/view.go#L114-L132
```
...
```go reference
https://github.com/adminoid/cosmos-sdk/blob/v0.46.0/store/iavl/store.go#L37-L40
```
...
```go reference
https://github.com/adminoid/cosmos-sdk/blob/v0.46.0/store/dbadapter/store.go#L14-L17
```
...
```go reference
https://github.com/adminoid/cosmos-sdk/blob/v0.46.0/store/transient/store.go#L16-L19
```
...
```go reference
https://github.com/adminoid/cosmos-sdk/blob/v0.46.0/x/params/types/subspace.go#L21-L31
```

Transient stores are typically accessed via the [`context`](./02-context.md) via the `TransientStore()` method:

```go reference
https://github.com/adminoid/cosmos-sdk/blob/v0.46.0/types/context.go#L264-L267
```
...
```go reference
https://github.com/adminoid/cosmos-sdk/blob/v0.46.0/store/cachekv/store.go#L27-L35
```
...
Cosmos SDK applications use [`gas`](../basics/04-gas-fees.md) to track resources usage and prevent spam. [`GasKv.Store`](https://github.com/adminoid/cosmos-sdk/blob/v0.46.0/store/gaskv/store.go) is a `KVStore` wrapper that enables automatic gas consumption each time a read or write to the store is made. It is the solution of choice to track storage usage in Cosmos SDK applications.

```go reference
https://github.com/adminoid/cosmos-sdk/blob/v0.46.0/store/gaskv/store.go#L11-L17
```
...
```go reference
https://github.com/adminoid/cosmos-sdk/blob/v0.46.0/store/types/gas.go#L219-L228
```
...
```go reference
https://github.com/adminoid/cosmos-sdk/blob/v0.46.0/types/context.go#L259-L262
```

In this case, the default gas configuration is used:

```go reference
https://github.com/adminoid/cosmos-sdk/blob/v0.46.0/store/types/gas.go#L230-L241
```
...
```go reference
https://github.com/adminoid/cosmos-sdk/blob/v0.46.0/store/tracekv/store.go#L20-L43
```
...
```go reference
https://github.com/adminoid/cosmos-sdk/blob/v0.46.0/store/prefix/store.go#L16-L22
```
...
Additional information about state streaming configuration can be found in the [store/streaming/README.md](https://github.com/adminoid/cosmos-sdk/tree/v0.46.0/store/streaming).

```go reference
https://github.com/adminoid/cosmos-sdk/blob/v0.46.0/store/listenkv/store.go#L11-L18
```
...
These types use the new `db` sub-module of Cosmos-SDK (`github.com/adminoid/cosmos-sdk/db`), rather than `tmdb` (`github.com/tendermint/tm-db`).
